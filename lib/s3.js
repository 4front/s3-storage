...
S3Storage.prototype.readFileStream = function(filePath, useFallback) {
  ...
  if (useFallback === true && this._s3Fallback) {
    if (this.options.metrics) {
      this.options.metrics.increment('read-fallback-storage');
    }
    if (this.options.logger) {
      this.options.logger.info('Object read from fallback storage', {
        key: key,
        code: 'readFallbackStorage'
      });
    }
  ...
  readStream
    ...
    .on('end', function() {
      self.options.metrics.timing('s3-storage-read', perfnow() - startTime);
    });
...
S3Storage.prototype.fileExists = function(filePath, callback) {
  ...
  this._s3.headObject({Bucket: this.options.bucket, Key: key}, function(err) {
    if (err && err.code === 'NotFound') {
      ...
      if (self._s3Fallback) {
        var fallbackBucket = self.options.fallback.bucket;
        self._s3Fallback.headObject({Bucket: fallbackBucket, Key: key}, function(fallbackErr) {
          if (fallbackErr) {
