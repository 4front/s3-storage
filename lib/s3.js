...
S3Storage.prototype.fileExists = function(filePath, callback) {
  ...
  this._s3.headObject({Bucket: this.options.bucket, Key: key}, function(err) {
    if (err && err.code === 'NotFound') {
      // If the file was not found, trigger the lazy replicator in case
      // the file does exist in the master region but for whatever reason
      // did not successfully replicate to this region.
